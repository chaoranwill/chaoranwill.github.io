...
- | :-: | :-: |:-: |:-: | -: 
HTML | 技术 | 浏览器/webview | 开发工具+框架+模块管理+任务管理（gulp）+浏览器兼容+版本迭代升级等 | 浏览器渲染时可能卡顿
小程序 | 应用 | 微信（APP）| 开发者工具+规范的开发标准+自定义WXML/WXSS+不考虑兼容性 | 更多系统权限：网络通信状态+数据缓存能力等 | 直接在微信上运行（无浏览器渲染步骤），运行更加流畅
